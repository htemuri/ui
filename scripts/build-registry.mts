...
async function buildRegistryJsonFile(styleName: string) {
  ...
  // 5. Write temporary registry file needed by 8starlabs-ui build.
...
async function buildRegistry(styleName: string) {
  return new Promise((resolve, reject) => {
    // Use local 8starlabs-ui copy.
    const outputPath =
      styleName === "new-york-v4" ? `public/r/styles/${styleName}` : `public/r/${styleName}`
    const process = exec(
      `node ../../packages/8starlabs-ui/dist/index.js build registry-${styleName}.json --output ${outputPath}`
    )

    // exec(
    //   `pnpm dlx 8starlabs-ui build registry-${styleName}.json --output public/r/styles/${styleName}`
    // )
